...
struct AppTests {
    ...
    func getAllTodos() async throws {
        try await withApp { app in
            let sampleTodos = [Events(title: "sample1"), Events(title: "sample2")]
            try await sampleTodos.create(on: app.db)
            
            try await app.test(.GET, "todos", afterResponse: { res async throws in
                #expect(res.status == .ok)
                #expect(try res.content.decode([EventsDTO].self) == sampleTodos.map { $0.toDTO()} )
            })
        }
    }
    ...
    func createTodo() async throws {
        let newDTO = EventsDTO(id: nil, title: "test")
        ...
        try await withApp { app in
            try await app.test(.POST, "todos", beforeRequest: { req in
                ...
                #expect(res.status == .ok)
                let models = try await Events.query(on: app.db).all()
    ...
    func deleteTodo() async throws {
        let testTodos = [Events(title: "test1"), Events(title: "test2")]
        ...
        try await withApp { app in
            ...
            try await app.test(.DELETE, "todos/\(testTodos[0].requireID())", afterResponse: { res async throws in
                ...
                let model = try await Events.find(testTodos[0].id, on: app.db)
                #expect(model == nil)
            })
        }
    }
